...
type ShadowPolicySpec struct {
	...
	// foo is an example field of ShadowPolicy. Edit shadowpolicy_types.go to remove/update
	// +optional
...
type ShadowPolicyStatus struct {
	...
	// conditions represent the current state of the ShadowPolicy resource.
...
type ShadowPolicy struct {
	...
	// spec defines the desired state of ShadowPolicy
	// +required
	Spec ShadowPolicySpec `json:"spec"`

	// status defines the observed state of ShadowPolicy
	// +optional
	Status ShadowPolicyStatus `json:"status,omitzero"`
}
