...
import (
	...
	"github.com/neha874-ctrl/shadowcast/internal/controller"
...
func main() {
	...
	// Initialize Envoy xDS Snapshot Cache
	snapshotCache := cachev3.NewSnapshotCache(false, cachev3.IDHash{}, nil)
	// Set up ShadowPolicy Reconciler with SnapshotCache
	if err := (&controller.ShadowPolicyReconciler{
		Client:        mgr.GetClient(),
		Scheme:        mgr.GetScheme(),
		SnapshotCache: snapshotCache,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "shadowpolicy")
		os.Exit(1)
	}
	...
	// Run xDS gRPC Server in background
	go func() {
		if err := xds.RunServer(ctx, snapshotCache, 18000); err != nil {
			setupLog.Error(err, "Failed to run xDS gRPC server")
			os.Exit(1)
		}
	}()
